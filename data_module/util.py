...
def default_window_image(dcm, cfg):
    ...
    window_center, window_width, intercept, slope = [get_first_of_dicom_field_as_int(x) for x in dicom_fields]
...
def grayscale_win_img_blur(dcm, cfg):
    ...
    if cfg.win_img_type == "default":
        ...
        win_img = custom_window_image(dcm=dcm, window_center=cfg.window_center[0], window_width=cfg.window_width[0])
...
def rgb_comb_img_blur(win_img1, win_img2, win_img3):
    ...
    Returns:
        _type_: _description_
    """
    combined_blurred_img = np.array(
        [win_img1, win_img2, win_img3]
    ).transpose(1, 2, 0)
